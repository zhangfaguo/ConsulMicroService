...
namespace Zfg.Core.EventBus.Cap
{
    internal class CapCustomerSelector : ConsumerServiceSelector
    {
        ...
        private IEnumerable<ConsumerExecutorDescriptor> FindConsumersFromIocTypes(IServiceProvider provider)
        {
            ...
            foreach (ServiceDescriptor service in CapBusExtentions.ServiceCollection.Where((ServiceDescriptor o) => o.ImplementationType != null || o.ImplementationFactory != null))
            {
        ...
        {
            var list = base.FindConsumersFromControllerTypes();
        ...
        {
            List<ConsumerExecutorDescriptor> executorDescriptorList = new List<ConsumerExecutorDescriptor>();
            foreach (Type exportedType in Assembly.GetEntryAssembly()!.ExportedTypes)
            {
